    ...
    <div id="layoutAuthentication_content" class="container-fluid">
        <div class="card mb-4">
            <div class="card-header">
                <i class="fas fa-table mr-1"></i>Emploi du temps de groupe {{$emploi->groupe}}.
                <a href="/Emplois/{{$emploi->id}}">
                    <button  class="btn btn-primary active px-3 py-1 mb-0 float-right"  >Modifier</button>
                </a>
            </div>
                                    ...
                                    @foreach ($seances as $seance)
                                        @if ($seance->jour == ${'J'.$i})
                                            @if ($seance->heureD == ${'heureD'.$j})
                                                <b>{{$seance->module}}</b>
                                                <p class="small m-0">{{$seance->enseignant}}</p>
